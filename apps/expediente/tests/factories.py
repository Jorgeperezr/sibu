...
from apps.core.models import PeriodoAcademico, Seccion, Servicio
...
def crear_profesional(username, servicio, seccion, rol=Rol.PROFESIONAL):
    user = Usuario.objects.create_user(
        username=username, password="x", rol_principal=rol,
...
def crear_expediente(cedula="1104567890"):
    persona = Persona.objects.create(
        cedula=cedula, nombres="Test", apellidos="Paciente",
...
def crear_atencion(expediente, servicio, perfil):
    ...
    return Atencion.objects.create(
        expediente=expediente, servicio=servicio, profesional=perfil,
        fecha_hora=timezone.now(), motivo_consulta="prueba")
