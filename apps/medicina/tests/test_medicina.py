...
def escenario(db):
    ...
    CIE10.objects.get_or_create(codigo="J00", defaults={
        "descripcion": "Rinofaringitis aguda (resfriado común)"})
    CIE10.objects.get_or_create(codigo="R50.9", defaults={
...
def test_crear_atencion_medicina(escenario):
    hc = services.crear_atencion_medicina(
        expediente=escenario["exp"], profesional=escenario["medico"],
...
def test_agregar_diagnosticos_y_unico_principal(escenario):
    hc = services.crear_atencion_medicina(
        expediente=escenario["exp"], profesional=escenario["medico"], motivo="x",
    )
